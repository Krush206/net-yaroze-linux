...
target_vendor = sony
target_os = psx
target_canonical = mips-sony-psx
...
exec_prefix = $(prefix)/psx
bindir = $(exec_prefix)/bin
libdir = $(exec_prefix)/lib
tooldir = $(exec_prefix)

program_transform_name = -e s,^,psx-, 
...
SUBDIRS =                                 mmalloc libiberty opcodes bfd readline db tcl tk tclX itcl tix texinfo byacc flex bison binutils ld gas gcc sim gdb make patch send-pr gdbtest tgas etc expect dejagnu diff release utils findutils                                
OTHERS = 
...
CONFIG_ARGUMENTS = --host=psx --build=i686-linux-gnu --enable-multilib --with-cross-host=i686-linux-gnu   
