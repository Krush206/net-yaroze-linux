...
SUBDIRS =                                mmalloc libiberty opcodes bfd readline db tcl tk tclX itcl byacc flex bison binutils ld gas gcc sim gdb make patch send-pr gdbtest tgas etc expect dejagnu diff release utils findutils                               
OTHERS = 
...
CONFIG_ARGUMENTS = --host=psx --with-cross-host=i686-linux-gnu   
