...
static void add_prefix		PROTO((struct path_prefix *, char *, int, int *));
...
static void
add_prefix (pprefix, prefix, first, warn)
     struct path_prefix *pprefix;
     char *prefix;
     int first;
     int *warn;
     ...
     char **argv;
{
  ...
  if (gcc_exec_prefix)
    {
      add_prefix (&exec_prefixes, gcc_exec_prefix, 0, NULL_PTR);
      add_prefix (&startfile_prefixes, gcc_exec_prefix, 0, NULL_PTR);
    }
      ...
      while (1)
	{
	  if (*endp == PATH_SEPARATOR || *endp == 0)
		...
		nstore[endp-startp] = 0;
	      add_prefix (&exec_prefixes, nstore, 0, NULL_PTR);
  ...
  if (temp && *cross_compile == '0')
      ...
      while (1)
	{
	  if (*endp == PATH_SEPARATOR || *endp == 0)
		...
		nstore[endp-startp] = 0;
	      add_prefix (&startfile_prefixes, nstore, 0, NULL_PTR);
	      if (*endp == 0)
		break;
	      endp = startp = endp + 1;
	    }
      ...
      else if (argv[i][0] == '-' && argv[i][1] != 0)
	{
	  ...
	  switch (c)
		...
		if (p[1] == 0)
		  ...
		  value = p + 1;
		add_prefix (&exec_prefixes, value, 1, &warn_B);
		add_prefix (&startfile_prefixes, value, 1, &warn_B);
...
#ifndef OS2
  add_prefix (&exec_prefixes, standard_exec_prefix, 0, warn_std_ptr);
  add_prefix (&exec_prefixes, standard_exec_prefix_1, 0, warn_std_ptr);
#endif

  add_prefix (&startfile_prefixes, standard_exec_prefix, 0, warn_std_ptr);
  add_prefix (&startfile_prefixes, standard_exec_prefix_1, 0, warn_std_ptr);

  add_prefix (&include_prefixes,
	      concat (tooldir_base_prefix, "include", dir_separator_str, NULL_PTR),
	      0, NULL_PTR);

  tooldir_prefix = concat (tooldir_base_prefix, "psx",
  ...
  add_prefix (&startfile_prefixes,
	      ...
	      0, NULL_PTR);
  add_prefix (&include_prefixes,
	      concat (tooldir_prefix, "include", dir_separator_str, NULL_PTR),
	      0, NULL_PTR);
     ...
     char **argv;
{
  ...
  if (*cross_compile == '0')
    {
#ifdef MD_EXEC_PREFIX
      add_prefix (&exec_prefixes, md_exec_prefix, 0, NULL_PTR);
      add_prefix (&startfile_prefixes, md_exec_prefix, 0, NULL_PTR);
#endif

#ifdef MD_STARTFILE_PREFIX
      add_prefix (&startfile_prefixes, md_startfile_prefix, 0, NULL_PTR);
#endif

#ifdef MD_STARTFILE_PREFIX_1
      add_prefix (&startfile_prefixes, md_startfile_prefix_1, 0, NULL_PTR);
#endif
	...
	add_prefix (&startfile_prefixes, standard_startfile_prefix, 0,
		    NULL_PTR);
      else
	{
	  if (gcc_exec_prefix)
      ...
      add_prefix (&startfile_prefixes, standard_startfile_prefix_1, 0,
		  NULL_PTR);
#if 0 /* Can cause surprises, and one can use -B./ instead.  */
      add_prefix (&startfile_prefixes, "./", 0, NULL_PTR);
#endif
    }
  else
    {
