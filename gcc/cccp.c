...
#ifdef KANJI
#define IS_SJIS1(X)		(((X) >= 0201 && (X) <= 0237) || \
				 ((X) >= 0340 && (X) <= 0374))
/* S-JIS 1st byte code.  */
static int sjis1 = 0;
#endif /* KANJI (SJIS) */
     ...
     int len;
{
  ...
  while (left > 0) {
    ...
    if (nchars == 0)
      ...
      for (xlen = nchars; xlen > 0; --xlen) {
	if (xlen >= 2 && sp[0] == '\r' && sp[1] == '\n') {
#if 0	/* CRLF code (input-file ONLY)  */
	  /* output filecoding CRLF (DOS) */
	  crlf = 1;
#endif
	  --xlen;
	  sp++;
...
#if 1					/* PSX patch */
static void * safe_memchr PROTO((void *, int, int));

static void *
safe_memchr(ptr, ch, len)
    void *ptr;
    int ch;
    int len;
{
     ...
     int len;
{
  ...
  if (crlf) {
    /* output filecoding CRLF (DOS) */
    while (len > 0) {
      char *lbuf = safe_memchr(ptr, '\n', len);
      int written;

      if (lbuf == NULL) {
	written = write (desc, ptr, len);
      } else {
	written = write (desc, ptr, lbuf - ptr);
      }
      if (written < 0) {
#ifdef EINTR
	if (errno == EINTR)
